...
class LabelInferenceThreatModel(TrainableThreatModel):
    ...
    def _generate_samples(
        ...
        if self.multiple_label_mode:
            ...
            warnings.warn(
                        "The following code has not been adapted to return less samples than available in memory. Please check implementation."
                    )

        # TODO take multi-label mode (see above) into account
        if num_samples<0:
            # if you want less datasets than there have been generated.
            return mem_datasets[:input_num_samples], mem_labels[:input_num_samples]
