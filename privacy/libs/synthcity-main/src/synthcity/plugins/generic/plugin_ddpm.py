...
class TabDDPMPlugin(Plugin):
    ...
    def _fit(self, X: DataLoader, *args: Any, **kwargs: Any) -> "TabDDPMPlugin":
        ...
        df = self.encoder.fit_transform(df, discrete_columns=[col_metadata['name'] for col_metadata in self.metadata if col_metadata['type'] == 'finite'])
