...
class tabddpm(Generator):
    """This generator is based on TabDDPM.
    It uses the implementation defined in synthcity, which directly implements the related paper.
    """
    ...
    def fit_generate(self, dataset, metadata, size, seed):
        tabddpm = Plugins().get("ddpm", random_state=seed)
        tabddpm.fit(dataset)
        return tabddpm.generate(count=size).dataframe()
