...
parser.add_argument("--generate_datasets", type = str2bool, default = 'True',
                    help = "Whether to generate train/test synthetic datasets or not.")
...
def main():
    datasets_path = Path(f'./generated/seed_{SEED}_generator_{NAME_GENERATOR}')
    ...
    if GENERATE_DATASETS:
        ...
        print(f'Saving datasets to {datasets_path}...')
        dump_pickle(datasets_train, datasets_path/'datasets_train.pkl')
        dump_pickle(labels_train, datasets_path/'labels_train.pkl')
        dump_pickle(datasets_train_utility, datasets_path/'datasets_train_utility.pkl')
        dump_pickle(datasets_test, datasets_path/'datasets_test.pkl')
        dump_pickle(labels_test, datasets_path/'labels_test.pkl')
    else:
        print(f'Loading shadow datasets from {datasets_path}...')
        datasets_train = load_pickle(datasets_path/'datasets_train.pkl')
        labels_train = load_pickle(datasets_path/'labels_train.pkl')
        datasets_train_utility = load_pickle(datasets_path/'datasets_train_utility.pkl')
        datasets_test = load_pickle(datasets_path/'datasets_test.pkl')
        labels_test = load_pickle(datasets_path/'labels_test.pkl')
    ...
    n_query_features = 10000
    QUERY_FEATURE_EXTRACTORS = [('query', range(1, 10), n_query_features, {'categorical':(1,), 'continuous': (3,)})]
    ...
    args.n_query_features = n_query_features
    QUERY_OUTPUT_PATH = f'{OUTPUT_DIR}/{NAME_GENERATOR}_output_{datetime.datetime.now()}_query_{n_query_features}.csv'
